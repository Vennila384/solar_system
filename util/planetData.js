...
{
  let planet=planetName.toLowerCase();
    let planetDetail;
    let count=0;
    // for(let i=0;i<obj.length; i++){
    //     if(planetName.toLowerCase()==obj[i].name.toLowerCase())
    //     {
      var test="mercury";
      console.log("testing..")
      // console.log(obj[test].name);
            planetDetail={
                name:obj[planet].name,
                content:obj[planet].overview.content,
                source:obj[planet].overview.source,
                rotation:obj[planet].rotation,
                revolution:obj[planet].revolution,
                radius:obj[planet].radius,
                temperature:obj[planet].temperature,
                image:obj[planet].images.planet
            }
            count=1;
            // break;
        // }
    ...
    // }
    // else{
        callback(undefined,planetDetail);
    // }
}

function getExtraDetails(planetName,type,callback)
{
  let planet=planetName.toLowerCase();
  console.log(planet);
  console.log(type+" here");
    ...
    //     {
            ...
            {
              console.log("I am in the structure");
              planetDetails={
                name:obj[planet].name,
                content:obj[planet].structure.content,
                source:obj[planet].structure.source,
                rotation:obj[planet].rotation,
                revolution:obj[planet].revolution,
                radius:obj[planet].radius,
                temperature:obj[planet].temperature,
                image:obj[planet].images.internal
              }
            }
            else if(type=="geology")
            {
              planetDetails={
                name:obj[planet].name,
                content:obj[planet].geology.content,
                source:obj[planet].geology.source,
                rotation:obj[planet].rotation,
                revolution:obj[planet].revolution,
                radius:obj[planet].radius,
                temperature:obj[planet].temperature,
                image:obj[planet].images.geology
              }
            }
            else{
    ...
    {
      callback(undefined,planetDetails);
    }
}

module.exports={
    getPlanetData:getPlanetData,
    getExtraDetails:getExtraDetails
}